...
class ReviewAuditServiceTest {
    ...
    void saveWebhookEvent_accepted_savesWithAcceptedStatus() {
        ...
        assertThat(captor.getValue().getProcessingStatus()).isEqualTo(WebhookEventEntity.ProcessingStatus.ACCEPTED);
    ...
    void saveWebhookEvent_ignored_savesWithIgnoredStatus() {
        ...
        ArgumentCaptor<WebhookEventEntity> captor = ArgumentCaptor.forClass(WebhookEventEntity.class);
        verify(webhookEventRepository).save(captor.capture());
        assertThat(captor.getValue().getProcessingStatus()).isEqualTo(WebhookEventEntity.ProcessingStatus.IGNORED);
    }
    ...
    void createReviewRun_savesWithRunningStatus() {
        ...
        ArgumentCaptor<ReviewRunEntity> captor = ArgumentCaptor.forClass(ReviewRunEntity.class);
        verify(reviewRunRepository).save(captor.capture());
        assertThat(captor.getValue().getStatus()).isEqualTo(ReviewRunEntity.ReviewStatus.RUNNING);
    ...
    void saveLlmMessage_savesCorrectEntity() {
        ...
        service.saveLlmMessage("run-3", LlmMessageEntity.Stage.REVIEW, 0,
                LlmMessageEntity.MessageRole.ASSISTANT, null, "Всё выглядит хорошо.");
        ArgumentCaptor<LlmMessageEntity> captor = ArgumentCaptor.forClass(LlmMessageEntity.class);
